...
describe('Cross-Chain Simulation Metadata Tests', () => {
  describe('Simulation Result Structure Validation', () => {
    metadataTest(
      'should contain valid metadata for Arbitrum cross-chain simulations',
      async () => {
    ...
    metadataTest(
      'should contain valid metadata for Optimism cross-chain simulations',
      async () => {
    ...
    metadataTest(
      'should validate simulation timing and block metadata',
      async () => {
        const crossChainResult = await getArbDistroCrossChainResult();
  ...
  describe('Cross-Chain Simulation State Validation', () => {
    metadataTest(
      'should track simulation success/failure states',
      async () => {
        const crossChainResult = await getArbDistroCrossChainResult();
    ...
    metadataTest(
      'should handle missing or invalid simulation data gracefully',
      async () => {
  ...
  describe('Cross-Chain Dependencies Validation', () => {
    metadataTest(
      'should validate dependency tracking in cross-chain simulations',
      async () => {
        try {
          const crossChainResult = await getArbDistroCrossChainResult();
  ...
  describe('Cross-Chain Message Integrity', () => {
    metadataTest(
      'should maintain message integrity across simulation phases',
      async () => {
        const crossChainResult = await getArbDistroCrossChainResult();
    ...
    metadataTest(
      'should validate L2 address aliasing for Arbitrum',
      async () => {
        const crossChainResult = await getArbDistroCrossChainResult();
        ...
        if (
          ...
          if (arbSimulation) {
            ...
            expect(arbSimulation.job.l2FromAddress.toLowerCase()).toBe(
    ...
    metadataTest(
      'should validate L2 address preservation for Optimism',
      async () => {
        const crossChainResult = await getOptimismBridgeCrossChainResult();
